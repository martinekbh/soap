...
class BilateralSolver(object):
    ...
    def solve(self, x, w):
        ...
        for d in range(x.shape[-1]):
            yhat[..., d], info = cg(A, b[..., d], x0=y0[..., d], M=M, maxiter=self.params["cg_maxiter"], rtol=self.params["cg_tol"])
        xhat = self.grid.slice(yhat)
        return xhat

def bilateral_solver_output(img, target, sigma_spatial = 24, sigma_luma = 4, sigma_chroma = 4) : 
    
    if isinstance(img, Image.Image): reference = np.array(img)
    elif isinstance(img, Tensor): reference = img.permute(1,2,0).numpy()
    else: reference = np.array(img)
